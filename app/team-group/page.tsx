...
export default function Page({ searchParams }: { searchParams: { data: string } }) {
    ...
    const addNewTaskFunc = () => {
        router.push(`/addRole?team_id=${encodeURIComponent(searchParams.data)}`)
    }
    ...
    return (
        ...
        {!loading && membersData && authData && (
            <>
            <Navbar />
            <div className="flex flex-col sm:flex-row lg:flex-row md:flex-row xl:flex-row justify-start items-start">
                <div className="flex flex-col justify-center w-full sm:w-1/2 lg:w-1/2 md:w-1/2 xl:w-1/2">
                    <div className='flex flex-col items-center justify-center'>
                        <h1 className="text-3xl text-center font-bold font-mono mb-8 mt-9">{teamName}</h1>
                    ...
                    <div className='flex px-5 justify-center sm:justify-center'>
                    {(owner || admin) && (
                        ...
                        <button className='flex py-2 px-7 rounded-lg mr-1 bg-synchBlue-50 hover:bg-synchBlue-150' onClick={addNewTaskFunc}>Add Role</button>
                ...
                <div className='flex justify-end mt-5 mr-5 w-full sm:w-1/2 md:w-1/2 lg:w-1/2 xl:w-1/2'>
                    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-4 w-full sm:ml-20 px-1 lg:ml-5">
                    {(tasks.length < 1) && (
                        <div className='flex'>
                            <h1 className='text-2xl font-mono'>No Roles available...</h1>
                        </div>
                    )}
                    {tasks.map((task: any) => (
                        <div key={task.task_id} className="bg-synchGray-100 p-4 rounded-lg w-[100%] sm:w-[100%] xl:w-[100%]">
